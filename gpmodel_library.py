...
class GPModel(object):
    ...
    def __init__(self, ranges, lengthscale, variance, noise = 0.0001, dimension = 2, kernel = 'rbf'):
        ...
        if kernel == 'rbf':
            ...
            # self.kern = gpflow.kernels.RBF(self.dim, lengthscales = lengthscale, variance = variance) 
        else:
    ...
    def predict_value(self, xvals, include_noise = False):
        ...
        # mean, var = self.model.predict_f(xvals)
...
class FlowGPModel(GPModel):
    ...
    def __init__(self, ranges, lengthscale, variance, noise = 0.0001, dimension = 2, kernel = 'rbf'):
        ...
        super(FlowGPModel, self).__init__(ranges, lengthscale, variance, noise, dimension, kernel)
    ...
    def add_data(self, xvals, zvals):
        ...
        # If the model hasn't been created yet (can't be created until we have data), create GPy model
        # self.model = GPy.models.GPRegression(np.array(self.xvals), np.array(self.zvals), self.kern, noise_var = self.noise)
        self.model = gpflow.models.GPR(np.array(self.xvals), np.array(self.zvals), self.kern)
        self.model.likelihood.variance = self.noise

    def posterior_samples(self, xvals, size=10, full_cov = True):
        fsim = self.model.predict_f_samples(xvals, size).reshape((-1, 1))
        return fsim
...
class SubsampledGPModel(OnlineGPModel):
    ''' This class inherits from the GP model class
        ...
        Woodbury-Morrison formula by modifying the Posteior class from the GPy Library 
    '''
    def __init__(self, ranges, lengthscale, variance, noise = 0.0001, dimension = 2, kernel = 'rbf', max_size = 20, neighbor_radius = 1.00, val_eps = 2.00):
