...
class Nonmyopic_Robot(Robot):
    ...
    def __init__(self, sample_world, start_loc = (0.0, 0.0, 0.0), ranges = (-10., 10., -10., 10.), kernel_file = None, 
            ...
            evaluation = None , f_rew = 'mean', computation_budget = 60, rollout_length = 6, input_limit = [0.0, 10.0, -30.0, 30.0],
             sample_number= 10, step_time = 5.0):
        ''' Initialize the robot class with a GP model, initial location, path sets, and prior dataset'''
        self.ranges = ranges
        self.eval = evaluation
        self.loc = start_loc # Initial location of the robot      
...
class Planning_Result():
    def __init__(self, planning_type, ranges, start_loc, input_limit, sample_number):

        self.type = planning_type
        if(planning_type=='coverage'):
            self.coverage_planning(ranges, start_loc)
        elif(planning_type=='non_myopic'):
            self.non_myopic_planning(ranges, start_loc, input_limit, sample_number)
        elif(planning_type=='myopic'):
            self.myopic_planning(ranges, start_loc)

    def myopic_planning(self, ranges_, start_loc_):
    ...
    def non_myopic_planning(self, ranges_, start_loc_, input_limit_, sample_number_):
        robot = Nonmyopic_Robot(sample_world = world.sample_value, 
                        ...
                        init_variance = 100.0, 
                        noise = 0.05,
                        path_generator = 'continuous_traj',
                        frontier_size = 20, 
                        horizon_length = 5.0, 
                        turning_radius = 0.5, 
                        sample_step = 2.0,
...
if __name__=="__main__":
    ...
    planning = Planning_Result(planning_type, ranges, start_loc, input_limit, sample_number)
